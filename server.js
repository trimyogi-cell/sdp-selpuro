...
app.put('/api/profil', (req, res) => { Object.assign(DB.profil, req.body); scheduleSave(); broadcast('profil', { action: 'update' }); res.json({ ok: true }); });
...
app.post('/api/users', (req, res) => {
  ...
  DB.users.push(user); scheduleSave(); broadcast('users', { action: 'add', id: user.id });
  res.json({ id: user.id });
});
app.put('/api/users/:id', (req, res) => {
  ...
  scheduleSave(); broadcast('users', { action: 'update', id });
  res.json({ ok: true });
});
app.delete('/api/users/:id', (req, res) => {
  const id = parseInt(req.params.id);
  DB.users = DB.users.filter(u => u.id !== id); scheduleSave(); broadcast('users', { action: 'delete', id });
  res.json({ ok: true });
});
app.delete('/api/users', (req, res) => {
  const keepId = req.query.keepId ? parseInt(req.query.keepId) : null;
  DB.users = keepId ? DB.users.filter(u => u.id === keepId) : [];
  scheduleSave(); broadcast('users', { action: 'deleteAll' }); res.json({ ok: true });
});
...
app.post('/api/change-password', (req, res) => {
  ...
  u.password = passBaru; scheduleSave(); res.json({ ok: true });
});
...
app.post('/api/siswa', (req, res) => {
  ...
  DB.siswa.push(s); scheduleSave(); broadcast('siswa', { action: 'add', id: s.id }); res.json({ id: s.id });
});
app.put('/api/siswa/:id', (req, res) => {
  ...
  scheduleSave(); broadcast('siswa', { action: 'update', id }); res.json({ ok: true });
});
app.delete('/api/siswa/:id', (req, res) => {
  const id = parseInt(req.params.id);
  DB.siswa = DB.siswa.filter(s => s.id !== id); DB.transaksi = DB.transaksi.filter(t => t.siswaId !== id);
  scheduleSave(); broadcast('siswa', { action: 'delete', id }); res.json({ ok: true });
});
app.delete('/api/siswa', (req, res) => { DB.siswa = []; DB.transaksi = []; scheduleSave(); broadcast('siswa', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/jenisbayar', (req, res) => {
  ...
  DB.jenisBayar.push(j); scheduleSave(); broadcast('jenisbayar', { action: 'add', id: j.id }); res.json({ id: j.id });
});
app.put('/api/jenisbayar/:id', (req, res) => {
  ...
  scheduleSave(); broadcast('jenisbayar', { action: 'update', id }); res.json({ ok: true });
});
app.delete('/api/jenisbayar/:id', (req, res) => { const id = parseInt(req.params.id); DB.jenisBayar = DB.jenisBayar.filter(j => j.id !== id); scheduleSave(); broadcast('jenisbayar', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/jenisbayar', (req, res) => { DB.jenisBayar = []; scheduleSave(); broadcast('jenisbayar', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/transaksi', (req, res) => {
  ...
  DB.transaksi.push(t); scheduleSave(); broadcast('transaksi', { action: 'add', id: t.id }); res.json({ id: t.id });
});
app.delete('/api/transaksi/:id', (req, res) => { const id = parseInt(req.params.id); DB.transaksi = DB.transaksi.filter(t => t.id !== id); scheduleSave(); broadcast('transaksi', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/transaksi', (req, res) => { DB.transaksi = []; scheduleSave(); broadcast('transaksi', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/stor', (req, res) => {
  ...
  DB.stor.push(s); scheduleSave(); broadcast('stor', { action: 'add', id: s.id }); res.json({ id: s.id });
});
app.delete('/api/stor/:id', (req, res) => { const id = parseInt(req.params.id); DB.stor = DB.stor.filter(s => s.id !== id); scheduleSave(); broadcast('stor', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/stor', (req, res) => { DB.stor = []; scheduleSave(); broadcast('stor', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/riwayat-wa', (req, res) => {
  ...
  DB.riwayatWa.push(r); scheduleSave(); res.json({ id: r.id });
});
