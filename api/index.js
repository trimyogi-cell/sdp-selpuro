...
app.put('/api/profil', (req, res) => { Object.assign(DB.profil, req.body); broadcast('profil', { action: 'update' }); res.json({ ok: true }); });
...
app.post('/api/users', (req, res) => {
  ...
  DB.users.push(user); broadcast('users', { action: 'add', id: user.id }); res.json({ id: user.id });
});
app.put('/api/users/:id', (req, res) => {
  const id = parseInt(req.params.id); const u = DB.users.find(x => x.id === id);
  ...
  broadcast('users', { action: 'update', id }); res.json({ ok: true });
});
app.delete('/api/users/:id', (req, res) => { const id = parseInt(req.params.id); DB.users = DB.users.filter(u => u.id !== id); broadcast('users', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/users', (req, res) => { const keepId = req.query.keepId ? parseInt(req.query.keepId) : null; DB.users = keepId ? DB.users.filter(u => u.id === keepId) : []; broadcast('users', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/change-password', (req, res) => {
  ...
  u.password = passBaru; res.json({ ok: true });
});
...
app.post('/api/siswa', (req, res) => {
  ...
  DB.siswa.push(s); broadcast('siswa', { action: 'add', id: s.id }); res.json({ id: s.id });
});
app.put('/api/siswa/:id', (req, res) => {
  ...
  broadcast('siswa', { action: 'update', id }); res.json({ ok: true });
});
app.delete('/api/siswa/:id', (req, res) => { const id = parseInt(req.params.id); DB.siswa = DB.siswa.filter(s => s.id !== id); DB.transaksi = DB.transaksi.filter(t => t.siswaId !== id); broadcast('siswa', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/siswa', (req, res) => { DB.siswa = []; DB.transaksi = []; broadcast('siswa', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/jenisbayar', (req, res) => {
  ...
  DB.jenisBayar.push(j); broadcast('jenisbayar', { action: 'add', id: j.id }); res.json({ id: j.id });
});
app.put('/api/jenisbayar/:id', (req, res) => {
  ...
  broadcast('jenisbayar', { action: 'update', id }); res.json({ ok: true });
});
app.delete('/api/jenisbayar/:id', (req, res) => { const id = parseInt(req.params.id); DB.jenisBayar = DB.jenisBayar.filter(j => j.id !== id); broadcast('jenisbayar', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/jenisbayar', (req, res) => { DB.jenisBayar = []; broadcast('jenisbayar', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/transaksi', (req, res) => {
  ...
  DB.transaksi.push(t); broadcast('transaksi', { action: 'add', id: t.id }); res.json({ id: t.id });
});
app.delete('/api/transaksi/:id', (req, res) => { const id = parseInt(req.params.id); DB.transaksi = DB.transaksi.filter(t => t.id !== id); broadcast('transaksi', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/transaksi', (req, res) => { DB.transaksi = []; broadcast('transaksi', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/stor', (req, res) => {
  ...
  DB.stor.push(s); broadcast('stor', { action: 'add', id: s.id }); res.json({ id: s.id });
});
app.delete('/api/stor/:id', (req, res) => { const id = parseInt(req.params.id); DB.stor = DB.stor.filter(s => s.id !== id); broadcast('stor', { action: 'delete', id }); res.json({ ok: true }); });
app.delete('/api/stor', (req, res) => { DB.stor = []; broadcast('stor', { action: 'deleteAll' }); res.json({ ok: true }); });
...
app.post('/api/riwayat-wa', (req, res) => {
  ...
  DB.riwayatWa.push(r); res.json({ id: r.id });
});
